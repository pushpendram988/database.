...
class AppDatabase {
  ...
  Future<bool> deleteNote({required String title , required String desc})async {
    var mainDB = await getDB();
    int rowEffect = await mainDB.delete(COLUMN_NOTE_ID
  ...
  void deleteOneNote(String title, String desc,)async{
    var mainDB = await getDB();
    mainDB.delete(TABLE_NOTE, where: "$COLUMN_NOTE_ID = ? ", whereArgs :[]);
  }
